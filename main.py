...
| 채점 항목 | 배점 | 세부 기준 |
|----------------------------|------|-------------------------------------------------------------------|
| 지도학습 vs 비지도학습 설명 | 5점 | 두 학습 방법의 정의·라벨 유무·대표 알고리즘을 명확히 설명했는가 |
| KNN 알고리즘 설명         | 5점 | 작동 원리(거리 기반), 주요 수식, 하이퍼파라미터(K) 언급 여부 |
| 분류 모델 성능 지표 설명   | 5점 | Accuracy·Precision·Recall 정의와 한계(불균형 데이터) 제시 여부 |
...
|--------------------------------|------|-----------|--------------------------------------------------------------|
| 지도학습 vs 비지도학습 설명 | 5점 | 5점 | 라벨 유무·대표 알고리즘(K‑Means)까지 정확히 언급 |
| KNN 알고리즘 설명           | 5점 | 4점 | 하이퍼파라미터(K) 언급 O, 수식 언급 X → 일부 부족 |
| 분류 모델 성능 지표 설명     | 5점 | 5점 | 세 지표 정의·한계(불균형 데이터)까지 명확히 설명 |

**근거 문장 & 선택 이유(Evidence + Why):**  
1. "지도학습은 라벨이 포함된 데이터를 사용해 모델을 학습시키는 방식이고, 비지도학습은 라벨 없이 데이터 내 패턴을 발견한다."  
   - **이유:** 라벨 유무를 명확히 구분해 Rubric 1의 핵심 요구사항을 충족.  
2. "K‑Nearest Neighbors(KNN)는 새로운 샘플과 학습 샘플 간의 거리를 계산하여 가장 가까운 K개의 이웃의 다수결로 클래스를 결정한다."  
   - **이유:** ‘거리 계산 → 다수결’ 작동 원리를 구체적으로 서술해 Rubric 2와 일치.  
3. "불균형 데이터에서는 Accuracy가 높아도 성능을 과대평가할 수 있다."  
   - **이유:** 지표 한계까지 언급해 Rubric 3의 ‘정의 + 한계’ 요구를 충족.

**총점: 14점 / 15점**  
**총평:** 핵심 개념 충실, KNN 수식 예시가 없어 1점 감점. 👏
...
- 각 근거 뒤에 ‘**이유:**’를 붙여 Rubric과의 대응 관계를 설명하세요.  
